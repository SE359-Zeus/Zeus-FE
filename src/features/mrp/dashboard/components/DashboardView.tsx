...
export function DashboardView() {
  ...
  const [isGeneratingPo, setIsGeneratingPo] = useState<Record<string, boolean>>({})
  ...
  const handleGeneratePO = async (orderId: string) => {
    setIsGeneratingPo(prev => ({ ...prev, [orderId]: true }))
    try {
      await apiPost(`/mrp/readiness/${orderId}/generate-po`)
      toast.success('PO Autogenerated', { description: `Successfully autogenerated PO for order ${orderId}.` })
      fetchMetrics()
      fetchTableData()
      if (expandedRows[orderId]) {
        setDetailData(prev => { const next = { ...prev }; delete next[orderId]; return next })
        setExpandedRows(prev => ({ ...prev, [orderId]: false }))
      }
    } catch (error) {
      toast.error('Action Failed', { description: 'Cannot activate auto PO generation feature.' })
    } finally {
      setIsGeneratingPo(prev => ({ ...prev, [orderId]: false }))
    }
  }
                          ...
                          <td colSpan={6} className="p-4 bg-mrp-app/40 relative">
                            <div className="border border-mrp-border/60 rounded-sm bg-mrp-panel/60 p-4 shadow-inner min-h-[120px]">
                              <div className="flex justify-between items-center mb-4">
                                ...
                                {hasShortage && (
                                  <button onClick={(e) => { e.stopPropagation(); handleGeneratePO(order.order_id) }} disabled={isGeneratingPo[order.order_id] || loadingDetails[order.order_id]} className="px-3 py-1.5 bg-mrp-primary text-white text-[12px] rounded-sm hover:bg-mrp-primary-hover flex items-center gap-1.5 disabled:opacity-50">
                                    {isGeneratingPo[order.order_id] ? <Loader2 size={12} className="animate-spin" /> : <Package size={12} />}
                                    Autogenerate Shortage PO
                                  </button>
                                )}
                              </div>
